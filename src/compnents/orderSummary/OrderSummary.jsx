        ...
        useEffect(()=> {
            ...
            (async() => {
                const _cartItems = await Promise.all( cartItems.map(async(item) => ({
                    ...item,
                    usdConversion: await handleConversion(item?.currency ??  item?.provision?.currency,"usd", item?.amount)
                })))
    ...
    <h3 className='text-2xl mb-10 font-semibold'>Order Summary</h3>
      ...
      {conversions?.map(item => (
            ...
            <div className='w-16 h-16 md:w-24 shrink-0 md:h-24 border rounded'>
                <img src={`/images/providers/${splitString(item?.provider ?? item?.product?.provider)}.webp`} className='w-full h-full object-contain' alt={item.provider} />
            </div>
            <div className='flex-1'>
            <p className='text-base font-semibold'>{item?.product?.provider}</p>
            <p className='text-base font-semibold'>{item?.provision?.name}</p>
            <p className='font-semibold font-semd text-gray-700'>{nairaFormat(item?.amount, item?.currency ?? item?.provision?.currency)}</p>
            

            <div className='font-medium text-gray-700 px-2 text-center'>             <CgArrowsExchangeAlt className='m-'/>        </div>
