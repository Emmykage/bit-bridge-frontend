...
const BillOrderDetails = ({
  ...
  return (
     ...
     <h2 className="text-2xl font-semibold mb-4 text-center">Billing Transaction</h2>
        <h2>
            ...
            <p className="md:w-60 border-b  border-gray-700 px-2 font-semibold">Token</p>
                <p className="flex-1 border-b  border-gray-700 px-2 font-bold text-2xl">{purchaseOrder?.token}</p>
        </div>
       }
        </h2>
    ...
    {
        purchaseOrder?.name && 
         <Detail label={"Customer Name"} value={purchaseOrder?.name} hidden={purchaseOrder?.service_type === "VTU" || purchaseOrder?.service_type === "VTU"} />

    }
       {
        purchaseOrder?.address && 
     
        <Detail label={"Address"} value={purchaseOrder?.address} hidden={purchaseOrder?.service_type === "VTU" || purchaseOrder?.service_type === "VTU"}  />
...

{
        purchaseOrder?.meter_number && 
         <Detail label={pickLabel(purchaseOrder?.service_type.toLowerCase())} value={purchaseOrder?.meter_number} />
