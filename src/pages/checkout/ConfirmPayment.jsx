...
const ConfirmPayment = () => {
 ...
 const refId = searchParams.get("paymentReference")
    ...
    console.log(purchaseOrder?.status, purchaseOrder?.status == "approved" || purchaseOrder?.status == "completed")
  ...
  return (<div className='bg-gray-900 h-screen text-white'>
    ...
    {/* https://bitbridgeglobal.com/app-redirect?paymentReference=bbg-1754300805 */}
          ...
          <div className={`${purchaseOrder?.status == "approved" || purchaseOrder?.status == "approved" ? "bg-green-200" : " bg-red-200"}  p-4`}>
            <p className={`${purchaseOrder?.status == "approved" || purchaseOrder?.status == "approved" ? "text-green-800" : " text-red-800"}  items-center flex gap-2 font-semibold text-center"`}>
        ...
        {refId.startsWith("fbg") ? <div className="bg-gray-900 text-white  flex items-center justify-center p-6">
      <div className="bg-gray-800 rounded-2xl shadow-lg p-6 w-full max-w-2xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          ...
          <Detail label="Amount" value={`₦${parseFloat(purchaseOrder.amount).toLocaleString()}`} />
          <Detail label="Bonus" value={`₦${parseFloat(purchaseOrder.bonus).toLocaleString()}`} />
          ...
          <Detail label="Created At" value={new Date(purchaseOrder.created_at).toLocaleString()} />
          <Detail label="Updated At" value={new Date(purchaseOrder.updated_at).toLocaleString()} />
        </div>
      </div>
    </div> :
    ...
    <BillOrderDetails purchaseOrder={purchaseOrder}/>
        ...
        <div className='bg-gray-800 rounded-2xl shadow-lg p-6 w-full max-w-2xl m-auto'>
