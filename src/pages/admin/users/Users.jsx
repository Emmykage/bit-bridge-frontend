              ...
              <table className="min-w-full bg-gray-300 border border-gray-200 rounded-md border-separate border-spacing-0 table-auto overflow-hidden">
                <thead className="bg-gray-900 text-alt">
                  <tr>
                    <th
                      scope="col"
                      className="sticky top-0 z-10 border-b border-gray-200/50  bg-opacity-75 py-3.5 pl-4 pr-3 text-left text-xs font-semibold backdrop-blur backdrop-filter sm:pl-6 lg:pl-8"
                    >
                      {' '}
                      EMAIL
                    </th>
                    <th
                      scope="col"
                      className="sticky top-0 z-10  border-b border-gray-200/50  bg-opacity-75 px-3 py-3.5  text-left text-xs font-semibold  backdrop-blur backdrop-filter sm:table-cell"
                    >
                      Role
                    </th>
                    <th
                      scope="col"
                      className="sticky top-0 z-10  border-b border-gray-200/50 bg- bg-opacity-75 px-3 py-3.5 text-left text-xs font-semibold  backdrop-blur backdrop-filter lg:table-cell"
                    >
                      Registered{' '}
                    </th>
                    <th
                  ...
                  {loading ? (
                    ...
                    users?.map((item) => (
                      <tr key={item?.id}>
                        ...
                        <td
                          className={`${item?.role === 'admin' && 'text-red-600'} font-semibold relative whitespace-nowrap border-b border-gray-200 py-3 pr-4 pl-3 text-left text-gray-900 text-sm sm:pr-8 lg:pr-8`}
                        >
                          {item?.role}
                        </td>
                        ...
                        <td className="whitespace-nowrap border-b border-gray-200 px-5 py-3 text-sm text-gray-600/90  font-medium ">
                          <NavLink to={`/admin/users/${item.id}`} className=" text-blue-600">
                            View
                          </NavLink>
                        </td>
                      </tr>
